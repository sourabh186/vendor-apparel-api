...
import { Apparel, OrderItem } from '../models/Apparel';
import fs from 'fs';
...
router.put('/:code/:size', (req, res) => {
    ...
    // Update data file
    fs.writeFileSync(dataPath, JSON.stringify({ apparel: apparelData }, null, 4));
...
router.put('/bulk', (req, res) => {
    const updates: { code: string; size: string; quantity: number; price: number }[] = req.body;
